...
public:
    ...
    bool CreateArenateam(Player* player, Creature* me)
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage("Команда арены 1x1 успешно создана!");
    ...
    bool OnGossipHello(Player* player, Creature* me)
    {
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage("Покинуть 1x1!");
            return true;
        }

        if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Встать в очередь арены 1x1", GOSSIP_SENDER_MAIN, 3, "Are you sure?", 0, false);
        else
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Встать в очередь на арену 1x1 (тестовый бой)", GOSSIP_SENDER_MAIN, 20);

        if(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == 0)
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Создать команду арены 1x1", GOSSIP_SENDER_MAIN, 1, "Create 1v1 arenateam?", sWorld->getIntConfig(CONFIG_ARENA_1V1_COSTS), false);
        else
        {
            if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Встать в очередь на арену 1x1 (рейтинговый бой)", GOSSIP_SENDER_MAIN, 2);
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Распустить команду арены 1x1", GOSSIP_SENDER_MAIN, 5, "Are you sure?", 0, false);
            }
        ...
        case 1: // Create new Arenateam
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("Необходим уровень %u+ чтобы создать команду арены 1x1", sWorld->getIntConfig(CONFIG_ARENA_1V1_MIN_LEVEL));
        ...
        case 2: // Join Queue Arena (rated)
            {
                if(Arena1v1CheckTalents(player) && JoinQueueArena(player, me, true) == false)
                    ChatHandler(player->GetSession()).SendSysMessage("Что-то пошло не так... Присоединиться к очереди.");
        ...
        case 4: // get statistics
            {
                ...
                if(at)
                {
                    ...
                    s << "\nИгр в неделю: " << at->GetStats().WeekGames;
                    s << "\nПобед в неделю: " << at->GetStats().WeekWins;
